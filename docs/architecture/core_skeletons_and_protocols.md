...
class Dispatcher:
    ...
    def __init__(
        self, policy: PolicyEngine, governor: ResourceGovernor, bus: "EventBus", adapters: "AdapterTable"
    ) -> None: ...
...
class EventBus:
    ...
    def subscribe(
        self, consumer_id: str, *, drop_policy: Literal["never", "drop_oldest"]
...
class ArmOutcome(Frozen):
    task_id: TaskId
    resolved: bool | None  # None = instrument error (excluded
    cost: BudgetDims
    instrument_error: str | None = None  # from denominator, B4)
...
class HarnessUnderTest(Protocol):
    ...
    async def attempt(
        self, task_id: TaskId, model_fingerprint: str, budget: BudgetDims, seed: int
    ) -> ArmOutcome: ...
...
class PairedRunner:
    ...
    async def run(
        self, family_id: str, manifest_hash: str, a: HarnessUnderTest, b: HarnessUnderTest
    ) -> PairedRun: ...
```
...
class WorkflowStep(Generic[In, Out]):
    ...
    input_type: type[In]
    output_type: type[Out]  # socket types — the schema
...
class TopologyValidator:
    ...
    def validate(self, topology_yaml: str, registry: Mapping[str, WorkflowStep]) -> "ValidatedTopology": ...
...
class WorkflowExecutor:
    ...
    async def execute(self, topo: "ValidatedTopology", task: Task, budget: BudgetDims) -> "RunResult": ...
```
