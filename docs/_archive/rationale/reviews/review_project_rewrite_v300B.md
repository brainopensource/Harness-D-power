...
> **Papel:** Prompt / RFP (Request for Proposal & Architecture Review) destinado ao **Tech Lead 2 (PhD) / Arquitetura de Software**.  
> **Diretriz de Tom:** Orientado estritamente a **sugestões, análises exploratórias, comparações sem viés e documentação de decisões (ADRs)**. Evitar dogmatismos ou imposições rígidas, solicitando ao Tech Lead 2 (PhD) um parecer totalmente honesto, de nível *Billion-Dollar Scale LLM Orchestrator & Coding Agent Design*.

---

## 1. OBJETIVO DO PROCESSO & FILOSOFIA DE DESIGN

O objetivo deste trabalho é guiar a transição da base prototípica atual (`src/sagiha/`) para a versão final de produção, batizada de **Aether v3.0.0B**.
...
* **Paper Trail & Padronização de Documentação (`docs/rationale/rewrite_b/`):** Todas as análises, especificações e Registros de Decisão de Arquitetura (ADRs) devem ser gravadas na pasta dedicada `docs/rationale/rewrite_b/` com o sufixo padronizado `rewrite_v300_<nome-informativo>_B.md`.
...
Para embasar a revisão do Tech Lead 2 (PhD), a inspeção empírica do código em `src/sagiha/` revela os seguintes pontos de atenção a serem diagnosticados e reavaliados para a versão v300B:
...
* **Fatos & Pontos de Análise:**
  ...
  - *Sugestão para o Tech Lead 2 (PhD):* Investigar quais portas devem ser consolidadas, simplificadas ou temporariamente removidas (regra de "pagar aluguel" do contrato) para manter o núcleo enxuto na V300B.
...
* **Fatos & Pontos de Análise:**
  ...
  - *Sugestão para o Tech Lead 2 (PhD):* Avaliar como arquitetar o loop de reparo dentro do `RunLoop` ou da camada de agência, garantindo que o agente aprenda com falhas intermediárias antes de encerrar a tarefa.
...
* **Fatos & Pontos de Análise:**
  ...
  - *Sugestão para o Tech Lead 2 (PhD):* Comparar e documentar os ganhos de isolar componentes de I/O e computação pesada local atrás de portas limpas, podendo usar Rust (`PyO3`) ou Go (via IPC/gRPC) onde for estritamente comprovado o gargalo de CPU/memória.

---

## 3. INSPIRAÇÃO E REFERÊNCIAS HISTÓRICAS (BIBLIOTECA DE PESQUISA)

As propostas presentes nos documentos de pesquisa em `docs/rationale/` e `docs/implementation/` servem como **fonte de inspiração e contexto histórico**, e **não como regras absolutas**. O Tech Lead 2 (PhD) tem total autonomia para aceitar, modificar ou rejeitar essas propostas com base em análises comparativas:
...
## 4. GUIA DE EXPLORAÇÃO ARQUITETURAL PARA O TECH LEAD 2 (PhD) (SUGESTÕES DE ANÁLISE)

Solicitamos ao Tech Lead 2 (PhD) uma análise profunda, sem viés e fundamentada nos seguintes tópicos-chave de engenharia:
...
* **Entregável Esperado:** `docs/rationale/rewrite_b/rewrite_v300_decisoes_runtime_B.md`
...
* **Entregável Esperado:** `docs/rationale/rewrite_b/rewrite_v300_mecanismo_edicao_B.md`
...
* **Entregável Esperado:** `docs/rationale/rewrite_b/rewrite_v300_contexto_memoria_B.md`
...
* **Entregável Esperado:** `docs/rationale/rewrite_b/rewrite_v300_seguranca_sandbox_B.md`
...
* **Entregável Esperado:** `docs/rationale/rewrite_b/rewrite_v300_autonomia_agi_B.md`

---

## 5. CHECKLIST DE ENTREGÁVEIS & ESTRUTURA DE ARQUIVOS (`docs/rationale/rewrite_b/`)

O Dev Tech Lead 2 (PhD) deverá criar os arquivos na pasta **`docs/rationale/rewrite_b/`** seguindo rigorosamente o padrão de nomes `rewrite_v300_<nome-informativo>_B.md`:

1. **`docs/rationale/rewrite_b/rewrite_v300_auditoria_sagiha_B.md`**  
   - Diagnóstico completo da base `src/sagiha/` classificando componentes em **Manter**, **Refatorar** ou **Eliminar/Substituir**.

2. **`docs/rationale/rewrite_b/rewrite_v300_decisoes_runtime_B.md`**  
...
3. **`docs/rationale/rewrite_b/rewrite_v300_decisoes_adr_B.md`**  
...
4. **`docs/rationale/rewrite_b/rewrite_v300_blueprint_arquitetura_B.md`**  
   - Blueprint completo da arquitetura do **Aether v3.0.0B**, especificando interfaces, portas hexagonais, diagrama de componentes e fluxos de dados.

5. **`docs/rationale/rewrite_b/rewrite_v300_roadmap_sprints_B.md`**  
