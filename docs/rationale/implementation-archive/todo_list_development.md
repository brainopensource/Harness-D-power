...
- [x] **OpenAI-Compatible Model Provider Adapter** (`Fast-follow`) — `OpenAIModelAdapter` in `adapters/model/openai.py`, 12 tests in `test_openai_adapter.py` all passing. Covers text responses, tool-call parsing, reasoning content, message history, HTTP error handling with retry, malformed JSON graceful degradation, `generate()` alias, and stream deferral. `composition.py` wires `live`/`record` modes via `_create_live_model_provider()`.
- [x] **`build_kernel` live/record binding** (D3) — `composition.py` binds `OpenAIModelAdapter` for `live` mode and wraps it in `CassetteModelProvider` for `record` mode. Tested in `test_openai_adapter.py::test_composition_live_mode` and `::test_composition_record_mode`.
...
- [x] **E0-Lite Evaluation Harness** (`Block 2`) — `Harvester` (`src/sagiha/e0/harvester.py`) + task runner (`src/sagiha/e0/runner.py`) + CLI (`sagiha harvest`, `sagiha bench`)
- [x] **Written Accept Predicate** (`Block 2`) — paired statistical comparison (`src/sagiha/e0/statistics.py`) with multiple-comparison correction (Holm-Bonferroni) and confidence interval estimation
- [x] **A/A Noise Floor Measurement** (`Block 2`) — local-first (`sagiha bench --aa`) measuring stochastic variance across un-mutated passes
- [x] **Resolve ADR-0015 Class B vs Class C** (`Block 2`) — Class B adopted for `brainopensource/Harness-D-power` with fallback for local repo harvesting
...
```sh
uv run pytest -q                                        # 88 passed, 91.77% coverage
...
The `OpenAIModelAdapter` is now implemented and tested (12 tests in `test_openai_adapter.py`),
and `composition.py` wires `live`/`record` modes. The cassette-driven exit test remains the
definition of done for Sprint 3a — live-model runs are now possible but are not gated by a sprint.

The next work on this page is **Block 2 (E0-lite)** — commit-replay harvester, benchmark runner,
A/A noise floor, and paired statistics.
