...
|:---|:---|:---|
| `default-off` — flip `SearchConfig.enabled` to `False` | ✅ Perfect | Matches honest-negative clause in [sprint_v2_s4_options.md](file:///home/rock_dev/Code/Harness/docs/implementation/sprint_v2_s4_options.md#L153), [STATUS.md](file:///home/rock_dev/Code/Harness/docs/STATUS.md#L168), and [s4-harvest-findings.md](file:///home/rock_dev/Code/Harness/docs/rationale/benchmarks/s4-harvest-findings.md#L120). Currently `True` at [config.py:310](file:///home/rock_dev/Code/Harness/src/sagiha/domain/config.py#L310) |
| `split-knobs` — prune vs repair semantics | ✅ Correct diagnosis | Defect #5 from [sprint_v2_s4_fixes.md](file:///home/rock_dev/Code/Harness/docs/implementation/sprint_v2_s4_fixes.md#L25). Current while-loop at [best_of_n.py:121-132](file:///home/rock_dev/Code/Harness/src/sagiha/adapters/search/best_of_n.py#L121-L132) confirms `not self._config.prune_on_first_gate_fail` makes `max_repair_rounds=2` dead code |
| `exporter-tests` | ✅ Valid gap | [eligibility.py](file:///home/rock_dev/Code/Harness/src/sagiha/outer_loop/export/eligibility.py) exists with 4-criteria `assess()` but grep confirms zero tests cover it |
| `ci-install` | ✅ Real issue | [ci.yml](file:///home/rock_dev/Code/Harness/.github/workflows/ci.yml) does `uv pip install --system` then `uv run` — mixed install paths |
...
| Leaving `noise-floor.md` as template | ✅ Correct | Template exists at [noise-floor.md](file:///home/rock_dev/Code/Harness/docs/rationale/benchmarks/noise-floor.md) |
...
- [config.py:310](file:///home/rock_dev/Code/Harness/src/sagiha/domain/config.py#L310): `SearchConfig.enabled` is currently `True`
- [config.py:493-507](file:///home/rock_dev/Code/Harness/src/sagiha/domain/config.py#L493-L507): `validate_security_invariants` enforces judge ≠ execution **only when `search.enabled=True`**
...
The **real problem** is: if someone sets `enabled=true` with default `prune_on_first_gate_fail=False`, repair is now ON, but the escalation ladder at [best_of_n.py:42-56](file:///home/rock_dev/Code/Harness/src/sagiha/adapters/search/best_of_n.py#L42-L56) still caps repair at 1 round because `escalate_after_failures=2` and `failures=round_+1=2` triggers escalation immediately after the first repair round. Your plan says raise `escalate_after_failures` to 3 — **you must do this in the same PR as the prune default flip**, or you ship a config where `prune=False` + `max_repair_rounds=2` still only runs 1 repair round. That's defect #6 from [sprint_v2_s4_fixes.md](file:///home/rock_dev/Code/Harness/docs/implementation/sprint_v2_s4_fixes.md#L28) surviving the fix.
...
Your plan says "Direct unit test: N candidates with costs → sum all, not winner." That's correct — [batch_cost](file:///home/rock_dev/Code/Harness/src/sagiha/adapters/search/best_of_n.py#L155-L175) sums all candidates.
...
2. `batch_cost` handles candidates cancelled by `cancel_on_clean_admit` (they have outcomes in `_outcomes` because `_run_and_release_one` stores them before the cancel scope fires at [best_of_n.py:235](file:///home/rock_dev/Code/Harness/src/sagiha/adapters/search/best_of_n.py#L235))
3. Candidates where `cost is None` are excluded (line [165](file:///home/rock_dev/Code/Harness/src/sagiha/adapters/search/best_of_n.py#L165)) — test this edge
...
Your plan says "extend `FakeWorktreeManager`" for the parallel release accounting test. But grep confirms **`FakeWorktreeManager` does not exist in `tests/`**. The [test_best_of_n.py](file:///home/rock_dev/Code/Harness/tests/unit/test_best_of_n.py) file at line 234 uses a `FakeWorktreeManager` that's defined locally in that test file — check whether it tracks allocate/release pairs.

Looking at the existing test at [line 234](file:///home/rock_dev/Code/Harness/tests/unit/test_best_of_n.py#L234), `FakeWorktreeManager` already exists as a test helper within the test module. You need to verify it has `allocate` and `release` tracking to assert every allocate has a matching release. If it doesn't track that yet, you need to add it.
...
Your plan says: "after system install use bare `python` / `sagiha` / `pytest` consistently." But look at [ci.yml](file:///home/rock_dev/Code/Harness/.github/workflows/ci.yml):
...
Both [sagiha.cpu.example.toml:27](file:///home/rock_dev/Code/Harness/sagiha.cpu.example.toml#L27) and [sagiha.gpu.example.toml:25](file:///home/rock_dev/Code/Harness/sagiha.gpu.example.toml#L25) currently say `enabled = true`. Your plan correctly says to sync these, but you should also update the **comments** in these files to explain that search is off by default because the empirical exit gate was not met.
...
Your plan mentions "Add a short closing section to s4-harvest-findings.md stating the default flip." The file currently ends at [line 152](file:///home/rock_dev/Code/Harness/docs/rationale/benchmarks/s4-harvest-findings.md#L152) with reproduction instructions. A §5 "Closeout" section recording the default flip, the `prune`/`repair` split, and the fact that `s4_bon_delta.md` is intentionally unpublished is appropriate. **Don't over-write it — 5 lines max.**

### Warning 2: STATUS Block 3 Wording Needs Care

The current STATUS at [line 97](file:///home/rock_dev/Code/Harness/docs/STATUS.md#L97) says search.enabled "remains an untested default." After your closeout it becomes `search.enabled=False`. The STATUS update should say:
...
The plan at [development_plan_v2.md](file:///home/rock_dev/Code/Harness/docs/implementation/development_plan_v2.md) has all S4 epics as `- [ ]`. Your closeout should mark S4.0–S4.4 mechanism as done with a note that the empirical gate is deferred. Don't touch S4 verification checkboxes — they're honestly not met.

### Warning 4: Test Count Monotonicity

Current baseline: **266 tests** per [STATUS.md:119](file:///home/rock_dev/Code/Harness/docs/STATUS.md#L119). Your closeout adds exporter tests + batch_cost test + parallel leak test + repair/prune split tests. New count should be **≥ 270**. State the expected floor in the PR.
