...
| **R5** | `runtime/` is empty | Tool execution lives in `adapters/tools/`. CAR's **R** has no code, so that half of the layer contract proves nothing | Move tool execution into `runtime/`, or drop **R** from the CAR story until it has code | Open — Sprint 3b |
...
| **R9** | Compaction | Prompt assembly ships with no compaction algorithm — only policy prose. The first assembler will invent ad-hoc truncation and break prefix stability | Specify three numbers (headroom %, keep-first-N, keep-last-M) in `prompt-architecture.md` before assembly grows | Open — Sprint 3b |
